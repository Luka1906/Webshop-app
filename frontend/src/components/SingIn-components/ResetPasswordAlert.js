...
const ResetPasswordMessage = ({ email }) => {
  return (
    ...
    <motion.div className="fixed top-0 left-0 w-screen h-screen z-20 " variants={resetSuccessAnimation} initial={"hidden"} animate={"visible"} >
      <Card className="fixed top-[20vh] w-[18rem] md:w-[25rem] transform translate-x-[-50%] left-[50%]  z-50 p-4 rounded-md">
        <div className="flex flex-col items-center gap-1.5  text-text-color text-xs md:text-sm ">
